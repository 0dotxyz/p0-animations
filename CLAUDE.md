...
4. **Reference assets** — token icons and logos can be:
   - **Remote URLs** (preferred for tokens): Use the Supabase-hosted icons:
     `https://xcdlwgvabmruuularsvn.supabase.co/storage/v1/object/public/p0-tokens/{mint_address}.png`
   - **Local files**: Drop into `public/assets/logos/` or `public/assets/icons/` and reference via `staticFile("assets/logos/drift.svg")`
   - Both remote URLs and `staticFile()` paths work with Remotion's `<Img>` component
